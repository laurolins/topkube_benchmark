...
# compute approx. density
ta_density = sapply(1:nrow(t), function (i) {
  if (is.na(t$ta_k[i])) return(NA)
  else if (is.na(t$sweep_num_ranks[i])) return(t$density[i])
  else if (t$sweep_num_ranks[i] > 1) return(t$ta_entries[i]/((t$num_ranks[i] - (t$sweep_num_ranks[i] - 1)) * t$keys[i]))
  else return(t$density[i])
})

t <- data.frame(t, 
                approx_density=ta_density, 
...
for (th in sort(unique(t$threshold))) {
  ...
  sel = t$threshold==th & !is.na(t$approx_density) & !is.na(t$cost)
  ...
  text(xinfo$lim[1],yinfo$lim[2],sprintf("hybrid %.3f",th),adj=c(0,1))
...
x = data.frame(d1 = t$density, d2 = t$approx_density)
